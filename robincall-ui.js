...
function sendSignalingMessage(payload) {
    ...
    const keyHash = 'webrtc_' + myPeerId + '_' + chId; // пишем в свой ключ
...
function startWebRTCPoll() {
    ...
    const listenKey = 'webrtc_' + theirPeerId + '_' + chId; // слушаем ключ собеседника
    let lastTimestamp = 0;
...
async function initWebRTC() {
    ...
    try { stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true }, video: false }); } catch (e) { rMsg('Ошибка доступа к микрофону!'); throw e; }
    pc = new RTCPeerConnection(rtcConfig);
    stream.getTracks().forEach(track => pc.addTrack(track, stream));
    pc.ontrack = (event) => { if (event.streams[0]) { remoteAudio.srcObject = event.streams[0]; remoteAudio.play().catch(() => {}); } };
    pc.onicecandidate = (event) => { if (event.candidate) sendSignalingMessage({ type: 'candidate', candidate: event.candidate }); };
...
window.addEventListener('beforeunload', () => { stopWebRTCPoll(); cleanupWebRTC(); P2PPong.destroy(); });
